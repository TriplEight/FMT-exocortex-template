...
- `day-rhythm-config.yaml → content_editor.enabled: false` (или ключ отсутствует) → пропустить весь пункт, секцию «Контент-план» в черновик не включать
- Иначе: выполни процесс из `prompts/content-plan.md`
