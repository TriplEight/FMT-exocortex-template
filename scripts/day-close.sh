...
# Slug вычисляется из полного пути IWE_ROOT (работает когда IWE не в ~/IWE).
# Переопределить можно через env IWE_MEMORY_SRC.
IWE_SLUG=$(echo "${IWE_ROOT:-$WORKSPACE_DIR}" | tr '/' '-')
MEMORY_SRC="${IWE_MEMORY_SRC:-$HOME/.claude/projects/${IWE_SLUG}/memory}"
